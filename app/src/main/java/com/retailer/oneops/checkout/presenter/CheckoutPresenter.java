...
public class CheckoutPresenter implements CheckoutPresenterInterface, Constant {
    ...
    public DisposableObserver<ResponseData<MOrder>> getPlaceOrderObserver() {
        return new DisposableObserver<ResponseData<MOrder>>() {
            @Override
            public void onNext(ResponseData<MOrder> response) {
                checkoutViewInterface.onSuccessfulPlaceOrder(response.getData(), response.getMessage());
            }
    ...
    private Observable deleteObservable(int id) {
        return APIClient.getClient(activity).create(APIInterface.class).deleteCartItem(new Session(activity).getString(AUTHORIZATION_KEY), id).subscribeOn(
                Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
    }

    /*private Observable placeOrderObservable(MOrderRequest mOrderRequest) {
    ...
    @Override
    public void onPlaceOrderClick(MOrderRequest mOrderRequest) {
        placeOrderObservable(mOrderRequest).subscribeWith(getPlaceOrderObserver());
    }

    private <T> Observable placeOrderObservable(MOrderRequest mOrderRequest) {
        return APIClient.getClient(activity).create(APIInterface.class).placeOrder(new Session(activity).getString(AUTHORIZATION_KEY), mOrderRequest).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
    }
